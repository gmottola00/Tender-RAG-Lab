...
async def create_lot(payload: LotCreate, db: AsyncSession = Depends(get_db_session)) -> LotOut:
    obj = await LotService.create(db, payload)
    return obj


@router.get("/{lot_id}", response_model=LotOut)
async def get_lot(lot_id: UUID, db: AsyncSession = Depends(get_db_session)) -> LotOut:
...
async def update_lot(lot_id: UUID, payload: LotUpdate, db: AsyncSession = Depends(get_db_session)) -> LotOut:
...
async def delete_lot(lot_id: UUID, db: AsyncSession = Depends(get_db_session)) -> None:
